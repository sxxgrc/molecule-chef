...
def get_train_and_val_product_property_datasets(params: Params, property_predictor, predictor_label_to_optimize):
    ...
    def transform_text_to_predictions(text_line, predict_func, predictor, predictor_label_to_optimize):
        ...
        property_predictions = [predict_func(predictor, predictor_label_to_optimize, mol_str) 
                                for mol_str in text_line.split('.')]
...
"""
Trains the Molecule Chef model as well as a property prediction model.
Trains property prediction model using the QED score for molecules and the score
generated by the given property_predictor, with optimization towards the provided label (1 or 0).
"""
def train_molecule_chef_qed_hiv(property_predictor, predictor_label_to_optimize):
    params = Params()
    ...
    # Set up data
    # == The property data
    train_prop_dataset, val_prop_dataset = (get_train_and_val_product_property_datasets(
